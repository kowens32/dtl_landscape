...
async function failOnMultipleErrors(text) {
  console.info(`WARN: ${text}`);
}

async function warnOnErrors(text) {
  console.info(`FATAL: ${text}`);
  setFatalError(text);
}
...
async function main () {
  ...
  tree.map(function(node) {
    if (node && node.item === null) {
      ...
      const getDalOwner = function() {
        if ((node.hasOwnProperty('delta_owner') && !node.delta_owner) ) {
          return 'Unknown';
        }

        if (node.delta_owner) {
          return node.delta_owner;
        }
        return 'Unknown';
      };
  ...
  if ((hasEmptyCrunchbase) && (process.env.CRUNCHBASE_KEY )){
    await reportFatalErrors();
    require('process').exit(1);
  }
  ...
  await Promise.mapSeries(itemsWithExtraFields, async function(item) {
    if (!item.unnamed_organization && item.crunchbase.indexOf('https://www.crunchbase.com/organization/') !== 0) {
      ...
      await warnOnErrors(`${item.name}  has a crunchbase ${item.crunchbase} which does not start with 'https://www.crunchbase.com/organization'`);
    }
  });
  if (hasBadCrunchbase) {
    await reportFatalErrors();
  ...
  const generateCrunchbaseSlugs = () => {
    if (typeof(crunchbaseData) != "undefined"){
    ...
    const urls = _.flatten(itemsWithExtraFields.map(({crunchbase, crunchbaseData}) => crunchbase ));
    const slugs = urls.filter((url) => url).map((crunchbaseUrl) => crunchbaseUrl.split("/").pop());
    return [...new Set(slugs)].sort()
  }
